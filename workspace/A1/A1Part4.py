...
def downsampleAudio(inputFile, M):
    ...
    down_sampled_x= hopSamples(x,M)
    #file name
    path_ends_on = inputFile.rfind("/")
    path = inputFile[:path_ends_on+1]
    name = inputFile[path_ends_on+1:]
    #don't write to the source path, but rather to the workspace
    output_name = "{}_downsampled.wav".format(name[:-4])
    new_rate = int(fs/M)
    wavwrite(down_sampled_x,new_rate,output_name)